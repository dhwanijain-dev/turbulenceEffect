...
uniform float u_turbulenceIntensity; // Changed to float
uniform vec2 u_mouse; // Added missing uniform

varying vec2 v_uv;

float hash(vec2 p){
  return fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453); // Fixed sinc to sin
}
...
void main() {
  ...
  float ratio = u_imageAspect.x / u_imageAspect.y;
  ...
  float mask = step(jaggedDist, u_radius.x);
  vec3 finalColor = mix(vec3(invertedGray), tex.rgb, 1.0 - mask);
